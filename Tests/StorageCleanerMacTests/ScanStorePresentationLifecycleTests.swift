...
final class ScanStorePresentationLifecycleTests: XCTestCase {
    ...
    func testCompletedScanPublishesTokenBoundPhasesWithoutSleepingBusinessWork() throws {
        ...
        let completion = try segment(source, from: "private func finishScanPresentation(", to: "private func waitForMinimumPresentationDuration(")
        XCTAssertFalse(completion.contains("Task.sleep"))
        XCTAssertFalse(completion.contains("await "))
        XCTAssertFalse(source.contains("static let scanningMinimum:"))
        XCTAssertFalse(source.contains("static let finalizingMinimum:"))
        XCTAssertFalse(source.contains("static let cleanupVerificationMinimum:"))
